...
class KDTree:
    ...
    def __init__(
        self, features_order="var", leaf_size="sqrt", distance=manhattan_distances
    ):
    ...
    def fit(self, X):
        ...
        if self.features_order == "var":
            ...
            sorted_features = np.random.choice(
                self.X.shape[1], self.X.shape[1], replace=False
            )
        else:
        ...
        if self.leaf_size == "sqrt":
            ...
            self.max_branch = int(
                np.log2(len(self.X) / np.log2(len(self.X))) / np.log2(2)
            )
        else:
    ...
    def _neighbour(self, args_predict, args_fit):
        distances = self.distance(self.X_predict[args_predict], self.X[args_fit])
...
class KDTreeForest:
    ...
    def __init__(self, n_trees=20, leaf_size="log", distance=manhattan_distances):
    ...
    def fit(self, X, validation_set=None):
        ...
        for k in range(self.n_trees):
            ...
            if validation_set is not None:
                ...
                print(
                    "Mean Absolute Error on Distance: %.3f (%.3f)"
    ...
    def predict(self, X):
        predict_min_distances = np.stack(
            [self.trees[i].predict(X).ravel() for i in range(len(self.trees))]
        )
