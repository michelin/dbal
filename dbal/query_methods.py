...
def aada_uncertainties(y_pred, y_disc):
    if len(y_pred.shape) == 1 or y_pred.shape[1] == 1:
        y_pred = y_pred.ravel()
        uncertainties = -(
            (1 - y_pred) * np.log(1 - y_pred + 1e-6) + y_pred * np.log(y_pred + 1e-6)
        )
    else:
...
class BaseQuery:
    ...
    def fit(self, Xt, Xs=None, ys=None, n_queries=None, sample_weight=None):
        ...
        self._fit(Xt=Xt, Xs=Xs, ys=ys, n_queries=n_queries, sample_weight=sample_weight)
        return self
    ...
    def fit_predict(self, Xt, Xs=None, ys=None, n_queries=None, sample_weight=None):
        self.fit(Xt=Xt, Xs=Xs, ys=ys, n_queries=n_queries, sample_weight=sample_weight)
...
class KMedoidsAccelerated(BaseQuery):
    ...
    def _fit(self, Xt, Xs, ys, n_queries, sample_weight, **fit_params):
        ...
        self.select_indexes = np.random.choice(len(Xt), batch_size_init, replace=False)
        self.weights = sample_weight
        if sample_weight is None:
            sample_weight = np.ones(len(Xt))
        queries = self._initialization(
            Xt[self.select_indexes],
            n_queries,
    ...
    def _initialization(self, Xt, n_queries, distance_to_nearest_src, weights):
        ...
        for i in range(n_queries):
            ...
            diff_matrix = np.clip(
                deltas_matrix - distance_matrix_initial_batch, a_min=0.0, a_max=None
            )
            ...
            if self.verbose == 1:
                if i % int(n_queries / 10) == 0:
                    print(
                        "Queries number: %i --- Objective: %.3f"
                        % (len(queries), deltas.mean())
                    )
            if self.verbose > 1:
                print(
                    "Queries number: %i --- Objective: %.3f"
    ...
    def _kmedoids(self, Xt, queries):
        ...
        self.distance_to_nearest_center = np.min(
            np.concatenate(
                (self.distance_to_nearest_src.reshape(-1, 1), self.distance_to_centers),
                axis=1,
            ),
            axis=1,
        )
        ...
        if self.weights is None:
            ...
            self.objective = (
                self.distance_to_nearest_center.ravel() * self.weights.ravel()
            ).mean()
        epoch = 0
        ...
        while previous_objective - self.objective > self.tol and epoch < self.max_iter:
            ...
            for k in range(len(self.centers_index)):
                if self.weights is None:
                    best_mean = self.distance_to_nearest_center[
                        self.cluster_indexes == k
                    ].mean()
                else:
                    best_mean = (
                        self.distance_to_nearest_center[
                            self.cluster_indexes == k
                        ].ravel()
                        * self.weights[self.cluster_indexes == k].ravel()
                    ).mean()
                argmin = self._compute_cluster_sums(
                    Xt[self.cluster_indexes == k], k, best_mean
                )
                if argmin is not None:
                    self.centers_index[k] = np.argwhere(
                        self.cluster_indexes == k
                    ).ravel()[argmin]
                    change_mask[k] = True

            if change_mask.sum() >= 1:
                self.distance_to_centers[:, change_mask] = self.distance(
                    Xt, Xt[self.centers_index[change_mask]]
                )
    ...
    def _compute_cluster_sums(self, Xt_k, k, best_mean):
        ...
        batch_indexes = np.random.choice(len(Xt_k), len(Xt_k), replace=False)
        ...
        while batch_size * i < len(Xt_k) and keep_indexes.sum() > batch_size:
            ...
            if i == 0:
                cluster_distances = self.distance(
                    Xt_k[keep_indexes],
                    Xt_k[batch_indexes[batch_size * i : batch_size * (i + 1)]],
                )
                if self.weights is None:
                    thresholds[1] = np.mean(cluster_distances, 0).min()
                    argmin_first_batch = batch_indexes[
                        np.argmin(np.mean(cluster_distances, 0))
                    ]
                else:
                    weights_vector = self.weights[self.cluster_indexes == k].reshape(
                        -1, 1
                    )
                    weights_matrix = np.repeat(
                        weights_vector, cluster_distances.shape[1], 1
                    )
                    thresholds[1] = np.mean(cluster_distances * weights_matrix, 0).min()
                    argmin_first_batch = batch_indexes[
                        np.mean(cluster_distances * weights_matrix, 0).argmin()
                    ]
            else:
            ...
            if self.weights is None:
                ...
                weights_vector = self.weights[self.cluster_indexes == k][
                    batch_indexes[: batch_size * (i + 1)]
                ].reshape(1, -1)
            ...
            thresholds[2] = (
                cluster_mean + (2 * cluster_std) / np.sqrt(batch_size * (i + 1))
            )[argmin]

            keep_mask = cluster_mean - (2 * cluster_std) / np.sqrt(
                batch_size * (i + 1)
            ) < np.min(thresholds)
        ...
        if batch_size * i < len(Xt_k):
            if keep_indexes.sum() >= 1:
                if i == 0:
                    cluster_distances = self.distance(
                        Xt_k[keep_indexes], Xt_k[batch_indexes[batch_size * i :]]
                    )
                else:
                    cluster_distances = np.concatenate(
                        (
                ...
                if self.weights is None:
                    ...
                    weights_matrix = np.repeat(
                        weights_vector, len(cluster_distances), 0
                    )
                    cluster_mean = np.mean(cluster_distances * weights_matrix, 1)
                thresholds[2] = cluster_mean.min()
                argmin_cluster_mean = np.argwhere(keep_indexes).ravel()[
                    np.argmin(cluster_mean)
                ]
            else:
                thresholds[2] = np.inf
...
class KMedoidsQuery(BaseQuery):
    """
    K-Medoids based active-learning query-strategy for domain adaptation.
    This class implements a query selection method that iteratively selects target domain
    samples to label based on a k-medoids clustering approach.

    Parameters
    ----------
    distance : callable, default=manhattan_distances
        Distance function to compute pairwise distances between samples.
        Should accept two arrays and return a distance matrix.
    nn_algorithm : str, default="brute"
        Algorithm for nearest neighbor computation. Options are:
        - "brute": Brute force computation of all pairwise distances
        - "kd-trees": Use KDTreeForest for efficient nearest neighbor search
    **kwargs : dict
        Additional keyword arguments passed to the underlying algorithm
        (e.g., KDTreeForest parameters when nn_algorithm="kd-trees").
    Attributes
    ----------
    distance : callable
        The distance function used for computing pairwise distances.
    nn_algorithm : str
        The nearest neighbor algorithm being used.
    kwargs : dict
        Additional parameters for the algorithm.
    queries : list
        Indices of selected query points in the target domain.
    deltas : ndarray
        Minimum distances from each target point to source domain or selected queries.
    kdtf : KDTreeForest, optional
        KDTreeForest instance when using "kd-trees" algorithm.
    Methods
    -------
    _fit(Xt, Xs, ys, n_queries, sample_weight)
        Fit the query strategy and select n_queries target samples to label.
    """

    def __init__(self, distance=manhattan_distances, nn_algorithm="brute", **kwargs):
        self.distance = distance
        self.nn_algorithm = nn_algorithm
        self.kwargs = kwargs
        super().__init__()
    ...
    def _fit(self, Xt, Xs, ys, n_queries, sample_weight):
        # compute minimum distance to source domain for every point in the target domain
        if Xs is None:
        ...
        # compute matrix of distances between all points in the target domain
        ...
        for i in range(n_queries):
            # compute closest distance to source domain + already selected queries, for every remaining points in the target domain
            self.deltas = np.min(
                np.stack(
            ...
            # compute, for every combination of target points, difference between the distance between the 1st and 2nd points, and the distance of the 2nd point to the closest source point
            deltas_matrix = np.resize(self.deltas, distance_matrix_tgt.shape)
            diff_matrix = np.clip(
                deltas_matrix - distance_matrix_tgt, a_min=0.0, a_max=None
            )
            # select next query as the target point that maximises distance to source domain while maintaining vicinity to other target points
            self.queries.append(
                diff_matrix.dot(sample_weight.reshape(-1, 1)).ravel().argmax()
            )

        self.deltas = np.min(
            np.stack(
                [deltas_src] + [distance_matrix_tgt[i] for i in self.queries], axis=1
            ),
            axis=1,
        )
        return self
...
class RandomQuery(BaseQuery):
    ...
    def _fit(self, Xt, Xs, ys, n_queries, sample_weight):
        ...
        sample_weight /= np.sum(sample_weight)
        self.queries = np.random.choice(
            len(Xt), n_queries, replace=False, p=sample_weight
        )
        return self
...
class KCentersQuery(BaseQuery):
    ...
    def _fit(self, Xt, Xs, ys, n_queries, sample_weight):
        ...
        for i in range(n_queries):
            ...
            deltas_tgt = self.distance(Xt, Xt[[argmax]])
            deltas = np.min(
                np.concatenate(
                    (deltas.reshape(-1, 1), deltas_tgt.reshape(-1, 1)), axis=1
                ),
                axis=1,
            ).ravel()
        return self
...
class BVSB:
    ...
    def __init__(self, get_base_model, **kwargs):
        tf = import_and_check_install("tensorflow")
    ...
    def embeddings(self, X, layer=-2):
        if isinstance(layer, str):
            embeddings = K.function(
                [self.model.layers[0].input], [self.model.get_layer(layer).output]
            )(X)[0]
        else:
            embeddings = K.function(
                [self.model.layers[0].input], [self.model.get_layer(index=layer).output]
            )(X)[0]
        return embeddings
...
class QBC:
    ...
    def fit(self, X, y, **fit_params):
        ...
        for model in self.models:
            index_boot = np.random.choice(len(X), size=len(X), replace=True)
    ...
    def embeddings(self, X, layer=-2):
        if isinstance(layer, str):
            embeddings = K.function(
                [self.models[0].layers[0].input],
                [self.models[0].get_layer(layer).output],
            )(X)[0]
        else:
            embeddings = K.function(
                [self.models[0].layers[0].input],
                [self.models[0].get_layer(index=layer).output],
            )(X)[0]
        return embeddings
